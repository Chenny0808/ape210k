...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    sni_loss = SNILossCompute(loss_fn)

    return compute, sni_loss
...
class SNILossCompute(nn.Module):
    def __init__(self, criterion):
        super(SNILossCompute, self).__init__()
        self.criterion = criterion
    ...
    def compute_loss(self, predict_out, target):
        out = predict_out
        ctype = Cast(torch.int64)
        tgt = ctype(target.view(-1))
        loss = self.criterion(out, tgt)
        stats = self.update_state(loss.clone(), predict_out, tgt)
        return loss, stats

    def update_state(self, loss, scores, target):
        pred = scores.max(1)[1]
        non_padding = target.ne(self.padding_idx)
        num_correct = pred.eq(target).masked_select(non_padding).sum().item()
        num_non_padding = non_padding.sum().item()
