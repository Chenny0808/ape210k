...
class GlobalAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    def __init__(self, dim, coverage=False, attn_type="dot",
                 attn_func="softmax", context=False):
    ...
    def forward(self, source, memory_bank, memory_lengths=None, coverage=None, copy_mask=None):
        """
        ...
        if copy_mask is not None:
            mask = copy_mask.eq(3).transpose(0, 1).unsqueeze(1)
            align.masked_fill_(1 - mask, -10)

        elif memory_lengths is not None:
        ...
        # concatenate
        if self.context:
            attn_h = c.view(batch, target_l, dim)
        else:
    ...
    * o_i = softmax(Wx_i) for x_i in X.
    ...
    def forward(self, x, lengths):
        ...
        Args:
            ...
            lengths: batch
        Output:
            global_info: hdim, batch
        """
        x = x.transpose(0, 1)
        scores = self.linear(x).view(x.size(0), x.size(1))

        x_mask = ~sequence_mask(lengths, max_len=scores.size(-1))

        scores.data.masked_fill_(x_mask.data, -float('inf'))

        alpha = F.softmax(scores, dim=-1)

        global_info = alpha.unsqueeze(1).bmm(x).squeeze(1)
        return global_info
