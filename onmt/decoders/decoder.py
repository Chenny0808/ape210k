...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
        ...
        self.embed_weightliner = nn.Linear(self._input_size, 1)
        ...
        if copy_attn and not reuse_copy_attn:
            ...
            self.copy_attn = GlobalAttention(
                hidden_size, attn_type=copy_attn_type, attn_func=attn_func, context=True
            )
        else:
            self.copy_attn = None
    ...
    def forward(self, tgt, memory_bank, memory_lengths=None, step=None, copy_mask=None):
        """
        Args:
        ...
        dec_state, dec_outs, attns = self._run_forward_pass(
            tgt, memory_bank, memory_lengths=memory_lengths, copy_mask=copy_mask)
...
class StdRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None, copy_mask=None):
        """
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None, copy_mask=None):
        ...
        # global_info = self.self_attention(memory_bank, memory_lengths)
        # if self.step == 0:
        #    input_feed = global_info


        for emb_t in emb.split(1):
            self.step += 1
            emb_t = emb_t.squeeze(0)
            weight = torch.sigmoid(self.embed_weightliner(emb_t))
            decoder_input = (weight + 0.1) * emb_t + (1 - weight + 0.1) * input_feed
            ...
            rnn_output, dec_state = self.rnn1(decoder_output, dec_state)
            if self.attentional:
                decoder_output, p_attn = self.attn1(
                    rnn_output,
                    memory_bank.transpose(0, 1),
                    memory_lengths=memory_lengths)
                attns["std"].append(p_attn)
            ...
            if self.copy_attn is not None:
                copy_context, copy_attn = self.copy_attn(
                    decoder_output, memory_bank.transpose(0, 1), copy_mask=copy_mask)

                attns["copy"] += [copy_attn]
                input_feed = input_feed + copy_context
